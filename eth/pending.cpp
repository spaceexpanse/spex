  ...
  for (unsigned i = 0; i < len; ++i)
    {
      ...
      cur.ns = dec.ReadString ();
      cur.name = dec.ReadString ();
      cur.mv = dec.ReadString ();

      /* Skip nonce and mover.  */
      dec.ReadUint (256);
      dec.ReadUint (160);
      ...
      if (amount > 0)
        outJson[receiver.GetChecksummed ()]
            = static_cast<Json::Int64> (amount);
      cur.metadata = Json::Value (Json::objectValue);
      cur.metadata["out"] = outJson;
